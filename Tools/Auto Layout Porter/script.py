...
def grad_to_rad(degrees):
    return [round(math.radians(float(deg.replace(',', '.'))), 5) for deg in degrees]

def scale_needed(scale):
    return any(float(scale[axis].replace(',', '.')) not in {0.0, 1.0} for axis in ['X', 'Y', 'Z'])
...
def convert_translate(translate):
    return [format(float(translate[axis].replace(',', '.')) / 10, '.5f').replace('.', ',') for axis in ['X', 'Y', 'Z']]
